...
class report_xls(report_sxw.report_sxw):
    ...
    def xls_write_row_with_indeks(self, ws, x, d, p, row_count, row_template, row_style,idx):
        r = ws.row(row_count)
        for col, size, spec in row_template:
            data = spec[4](x, d, p)
            style = spec[6] and spec[6] or row_style
    ...
    def xls_write_row_header_settings(self, ws, row_count, row_template, row_style=None, set_column_size=False):
        r = ws.row(row_count)
        for col, size, spec in row_template:
            data = ''
            if size != 1:
