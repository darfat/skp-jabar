...
from employee_realisasi_recapitulation_report_parser import employee_realisasi_recapitulation_report_parser

class employee_realisasi_recapitulation_report_xls_generator(report_xls):
	
    def generate_xls_report(self, parser, filters, obj, workbook):
	worksheet = workbook.add_sheet(('Rekap Realisasi Pegawai'))
	...
	# lamda d,f,p: is a function who has filter,data,parser as the parameters it is expected to the value of the column
	    ...
	    ('Title Period' ,  2, 0, 'text', lambda x, d, p: p.get_title_period('Laporan Rekapitulasi Realisasi Periode :',filters)),
	    
	    # Top Headers
	    ('Jumlah', 8, 100, 'text', lambda x, d, p: 'Jumlah Kegiatan Berdasar Status'),
	    ...
	    ('Realisasi', 1, 50, 'number', lambda x, d, p:  d['count_realisasi']),
	    ...
	    ('Banding', 1, 50, 'number', lambda x, d, p:  d['count_appeal']),
	    ('Verikasi BKD', 1, 50, 'number', lambda x, d, p:  d['count_evaluated']),
	    ('Ditolak BKD', 1, 50, 'number', lambda x, d, p:  d['count_rejected_bkd']),
	    ('Selesai', 1, 50, 'number', lambda x, d, p:  d['count_done']),
	    ('Nilai', 1, 50, 'number', lambda x, d, p:  d['nilai_akhir']),
        ...
        row_spec_value = ['NO','NIP','Nama Pegawai','Jabatan','Bidang','Biro','Nama OPD','User ID',
						 'Draft','Realisasi','Penilaian Atasan','Ditolak Atasan','Banding','Verikasi BKD','Ditolak BKD','Selesai','single_empty_column']
        ...
        row_count = 7
        result = parser.get_employee_realisasi_recapitulation_report_raw(filters);
        for employee_realisasi_recapitulation_data in result:
            # Write Rows
                style = row_normal_style
                if(row_count % 2 == 0):
                    style = row_normal_odd_style;
                self.xls_write_row(worksheet, filters, employee_realisasi_recapitulation_data, parser, row_count, row_template, style)
...
employee_realisasi_recapitulation_report_xls_generator(
    #name (will be referred from employee_realisasi_recapitulation_report.py, must add "report." as prefix)
    'report.employee.realisasi.recapitulation.xls',
    #model
    'employee.realisasi.recapitulation.report',
    #file
    'addons/df_report_employee_realisasi_recapitulation/report/employee_realisasi_recapitulation_report.xls',
    #parser
    parser=employee_realisasi_recapitulation_report_parser,
    #header
    header=True
)